...
function train_recur!(sol, n_batches, c_value, c_entropy)
    ...
    mb_idxs = Iterators.partition(env_idxs, envs_per_mb)
    ...
    for i ∈ 1:sol.n_epochs
        ...
        done_data = (mem.nex_done[_idxs] for _idxs ∈ mb_idxs)
...
function actor_loss(actor, S, A, ADV, P, DONE, ϵ)
    Π = actor(S, DONE)
    L = length(P)
...
critic_loss(critic, S, V, DONE) = sum(abs2, vec(critic(S, DONE)) .- V)

function surrogate_loss(net, S, A, ADV, V, P, DONE, ϵ)
    Π, V̂ = net(S, DONE)
    V̂ = vec(V̂)
    L = length(V)
    r_t = map(eachcol(Π), P, A) do col, vi, i
